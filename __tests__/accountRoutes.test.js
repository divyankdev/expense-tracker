...
describe('Account Endpoints', () => {
  ...
  it('should get all accounts for a user', async () => {
    // Note: You might need to authenticate this request depending on your middleware
    // For simplicity here, assuming no auth or auth is handled separately
    const res = await request(app).get(`/api/accounts?user_id=${testUser.user_id}`); // Assuming endpoint supports filtering by user_id

    expect(res.statusCode).toEqual(200);
    expect(res.body).toHaveProperty('status', 'success');
    expect(res.body.data).toBeInstanceOf(Array);
    // Add more specific assertions about the returned accounts
  });
  ...
  it('should get an account by ID', async () => {
    ...
    const res = await request(app).get(`/api/accounts/${testAccount.account_id}`);
    ...
    // Add more assertions for other account properties
  });
  ...
  it('should update an account by ID', async () => {
    ...
    expect(parseFloat(res.body.data.current_balance)).toEqual(updatedAccountData.current_balance); // Handle potential numeric type differences
  ...
  it('should delete an account by ID', async () => {
    ...
    const res = await request(app).delete(`/api/accounts/${testAccount.account_id}`);
    expect(res.statusCode).toEqual(200); // Or 204 No Content if preferred
    expect(res.body).toHaveProperty('status', 'success'); // Or check for no body with 204
  });
});