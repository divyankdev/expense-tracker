    ...
    try {
      ...
      if (!user) {
        ...
        const newUser = {
          ...
          // You might want to set a default password hash or a flag indicating Google sign-in
          password_hash: authService.generateRandomPasswordHash(), // Generate a random hash or use a specific indicator
          // Add any other necessary fields from the profile or defaults
          phone_number: null,
          date_of_birth: null,
          is_active: true, // Assuming new users are active
        };
        // Use userService.createUser to create the new user
        user = await userService.createUser(newUser);
        const insertValues = [newUser.email, newUser.first_name, newUser.last_name, newUser.profile_picture_url, newUser.password_hash];
        const { rows: newRows } = await query(insertSql, insertValues);
        user = newRows[0];
      }
